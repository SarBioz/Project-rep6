...
to_plot <- reductions %>%
  ...
  filter(sample_id %in% chimp_meta$sample_id) %>%
  left_join(chimp_meta, by = "sample_id")

#plot 10x nuclei by cluster
tmp <- chimp_meta %>% distinct(cluster, cluster_color)
...
tmp <- chimp_meta %>% distinct(donor, donor_color)
